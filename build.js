...
async function build() {
  ...
  const optimized = optimize(bundled.output[0].code);

  info('module', bundled.output[0].code, optimized.code);
  ...
  const optimizedUMD = optimize(UMD.output[0].code);

  info('umd', UMD.output[0].code, optimizedUMD.code);

  const ES5 = babel.transform(UMD.output[0].code, {
    sourceType: 'module',
  });
...
function info(name, code, optimized) {
  console.log(
    name,
    `${toByte(code.length)}`,
    `(${toByte(optimized.length)} minified, ${toByte(
      gzipSize.sync(optimized)
    )} gzip)`,
    '✔'
  );
}
    ...
    webpack(
      {
        ...
        plugins: [new ExtractTextPlugin('/vue-tampan.min.css')],
      },
      (error, stats) => {
